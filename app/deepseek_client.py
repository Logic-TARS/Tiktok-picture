...
def generate_caption(config: dict[str, Any], payload: dict[str, Any]) -> dict[str, Any]:
    ...
    body = {
        ...
        "messages": [
            {
                "role": "system",
                "content": "你是抖音二次元图集账号的中文文案助手。只输出 JSON。",
            },
    ...
    result = normalize_caption_result(parse_caption_json(content), payload)
...
def local_caption(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    return normalize_caption_result(result, payload)


def normalize_caption_result(result: dict[str, Any], payload: dict[str, Any]) -> dict[str, Any]:
    normalized = dict(result)
    normalized["title"] = normalize_title(str(result.get("title", "")).strip(), payload)
    normalized["body"] = str(result.get("body", "")).strip()
    normalized["hashtags"] = normalize_hashtags(result.get("hashtags") or [], limit=5)
    return normalized


def normalize_title(title: str, payload: dict[str, Any]) -> str:
