...
def publish_to_creator(job: dict[str, Any], config: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        if should_force_9x16(job, config):
            ...
            upload_paths, temp_upload_dir = prepare_9x16_uploads(upload_paths, mode=upload_mode)
            LOGGER.info(
        ...
        LOGGER.info("Uploaded %s file(s) for job %s", len(upload_paths), job["id"])
        ...
        return {
            "status": "need_manual",
            "message": f"素材和文案已填入创作者中心，请手动检查后点击发布。类型：{job.get('material_type')}。音乐：{music_status}",
...
def resolve_upload_input(page: Any, job: dict[str, Any], upload_selector: str) -> Any:
    ...
    if job.get("material_type") == "image_gallery":
        ...
        for selector in candidates:
            ...
            if locator.count():
                LOGGER.info("Using image gallery upload selector for job %s: %s", job["id"], selector)
                return locator
    ...
    candidates = [
        ...
        "input[type='file']:not([multiple])[accept*='video']",
    ]
    for selector in candidates:
        locator = page.locator(selector).first
        if locator.count():
            LOGGER.info("Using video upload selector for job %s: %s", job["id"], selector)
            return locator
